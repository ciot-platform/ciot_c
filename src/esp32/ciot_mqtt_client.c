...
#if CIOT_CONFIG_FEATURE_MQTT_CLIENT == 1 && defined(CIOT_PLATFORM_ESP32)
...
ciot_err_t ciot_mqtt_client_start(ciot_mqtt_client_t self, ciot_mqtt_client_cfg_t *cfg)
{
    ...
    const esp_mqtt_client_config_t mqtt_client_cfg = {
        ...
        .session.last_will.msg = base->cfg.has_last_will ? (const char*)base->cfg.last_will.payload.bytes : NULL,
        .session.last_will.msg_len = base->cfg.has_last_will ? base->cfg.last_will.payload.size : 0,
        .session.last_will.qos = base->cfg.has_last_will ? base->cfg.last_will.qos : 0,
        .session.last_will.topic = base->cfg.has_last_will ? base->cfg.last_will.topic : NULL,
        .session.last_will.retain = base->cfg.has_last_will ? base->cfg.last_will.retain : false,
        .session.keepalive = base->cfg.has_session ? base->cfg.session.keep_alive : 120,
        .session.disable_clean_session = base->cfg.has_session ? !base->cfg.session.clean_session : false,
#ifdef CONFIG_MBEDTLS_CERTIFICATE_BUNDLE
...
ciot_err_t ciot_mqtt_client_publish(ciot_mqtt_client_t self, char *topic, uint8_t *data, int size, int qos, bool retain)
{
    ...
    if (qos == 0)
    {
        err = esp_mqtt_client_publish(self->handle, topic, (char *)data, size, qos, retain);
        if (err == 0)
            ciot_mqtt_client_update_data_rate(self, size);
    }
    else
    {
        err = esp_mqtt_client_enqueue(self->handle, topic, (char *)data, size, qos, retain, false);
        if (err == 0)
            ciot_mqtt_client_update_data_rate(self, size);
    }
    return err == ESP_OK ? CIOT_ERR_OK : CIOT_ERR_FAIL;
...
static void ciot_mqtt_event_handler(void *handler_args, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_DATA:
        ...
        {
            ciot_iface_send_event_data(&base->iface, CIOT_EVENT_TYPE_MSG, (uint8_t*)mqtt_event->data, mqtt_event->data_len);
        }
        else
        {
